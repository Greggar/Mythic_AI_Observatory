...
async def _discover_instances() -> dict[str, dict[str, str]]:
    ...
    # Fallback: if Prometheus returned nothing, use localhost
    if not instances:
        instances["primary"] = {"instance": "127.0.0.1:9100"}
    return instances
...
async def collect_vitals() -> list[dict[str, Any]]:
    ...
    for instance, info in machine_map.items():
        ...
        # GPU stats only for the local machine
        if mid == LOCAL_HOSTNAME and _GPU_AVAILABLE:
        ...
        raw_map: dict[str, float] = {
            ...
            "diskSpace": ds_pct,
            **({"gpu": gpu_local['gpu_util'], "gpuMem": gpu_local['gpu_mem_pct'], "gpuTemp": gpu_local['gpu_temp']} if mid == LOCAL_HOSTNAME else {}),
        }
