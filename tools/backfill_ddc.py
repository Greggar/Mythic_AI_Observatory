#!/usr/bin/env python3
"""One-time backfill: classify all existing traces with DDC metadata."""
...
async def _classify_one(
    ...
    try:
        if data.get("ddc") and data["ddc"].get("prompt"):
            return line_idx, False, False

        trace = TraceSession(**data)
        ddc = await classify_ddc(trace.prompt, trace.output)

        if ddc.prompt or ddc.response:
            data["ddc"] = json.loads(ddc.model_dump_json())
...
async def main() -> None:
    ...
    print(f"{total} total, {skipped} already have DDC, {len(pending)} to classify")
    t0 = time.time()
