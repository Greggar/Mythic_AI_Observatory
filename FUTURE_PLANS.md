...
| 45 | **Play button per trace (History Trace Log)** — a play/replay affordance on each row in `TraceTable` (and/or constellation dot) that routes the trace straight into the shared analysis surface via the existing History→Trace bridge (`handleHistorySelect`/`handleReplay`). One click to re-open a trace's full intelligence panel, skipping the History→Trace tab dance. *(added 2026-08-27)* | — | ★☆☆ | 30 min |
| 46 | **Multi-select + group delete traces** — checkbox column in `TraceTable` (or constellation) to "check" several traces, then a bulk Delete button removes all selected via a backend batch-delete endpoint. Needs a new `POST/DELETE /api/traces/bulk-delete` (ids array) that removes each from `traces.jsonl` + the synesth cache + model-profile derived records, then refreshes all panels. Guard with a confirm dialog; protect against deleting the currently-active trace. *(added 2026-08-27)* | — | ★★☆ | 1.5 h |
| 47 | **Shared frontend network layer** — consolidated all 86 raw `fetch(` sites and every bespoke polling loop into `src/lib/api.ts` (`apiGet/apiPost/apiPut/apiDelete/apiBlob/apiUrl/ApiError`) and `src/lib/usePoll.ts` (`usePoll` continuous poller + `pollUntil` job poller). Zero raw `fetch(` remains outside the client module; only `LogTerminal`'s SSE `EventSource` still touches the network directly. `useWebSocket`'s telemetry poll, chat/orchestrate job polling, vitals/activity/metrics pollers, and all probe/suite/batch run loops now share one transport + error-normalization path. Verified: `tsc --noEmit` + `pnpm build` clean, ESLint −12 problems vs baseline (no new errors — 5 legacy debt patterns removed, explicit-`any` reduced 18→11 via a shared `TraceSummary` type), production service restarted, HTTP 200. ✓ *(2026-09-02)* | — | ★★☆ | 3 h |
...
| 8 | **Context carry-over** — v1: the existing memory-retrieval stage treats prior exchanges in the same chat as retrievable context (no raw history injection). v2: explicit truncated history injection per model family. ⚠ **live-observed failure 2026-08-06** (chat `53f3b337`): EX2 ("write the lyrical narrative poem you suggested on the topic we were discussing") generated a generic shadow-cartography poem with no Mars despite EX0's moons-of-Mars response ranking 0.6179 `used: true`; the #1 chunk (0.6466) was the *previous exchange's own meta-commentary* (self-reference pollution). **Root cause (code-read 2026-08-06): content was never delivered, not ignored.** Step-4 appended only a similarity-score summary to `context` (`orchestrator.py`); the actual chunk texts never reached the generator, and chat history was never loaded — each exchange ran in isolation. "used" meant "counted in the summary", not "injected". EX3 only "bridged" because the correction message itself named the topic. **Fix ✓ done (2026-08-06)**: (a) step-4 now injects the used chunks' content (truncated 700 chars, `[Memory Retrieval · rel x.xx]` lines) at `MEMORY_USE_THRESHOLD=0.15`; (b) chat traces now carry prior exchanges into context (`[Chat History]` block, newest-first within `CHAT_HISTORY_CHARS`/`CHAT_HISTORY_EXCHANGES` budgets). Verified live: an elliptical "poem on that topic now" in a fresh 2-exchange chat produced a Prometheus-grounded poem. **Still open**: retrieval-rank hygiene (deprioritize the immediately-preceding exchange / filter self-referential responses from the corpus) — the top-2 chunks were still cross-chat poems, rescued only by the history block. ⚠ **live-observed failure 2026-08-08 (post-fix, chat `1e8f0f2b`, backoffice/qwen3:latest)** — the delivery fix WORKS, but a NEW resolution gap surfaced at the *generation* stage. EX1 (`b622c2a2a70b`) "what do you know about the rings of Saturn?" → full overview; EX2 (`7dc55a3f09c5`) "What style of poetry would most suit them?" received the Saturn content in context (verified in `steps[5].context_assembled`, 5538 chars: `[Chat History]` block carries EX1's full rings output incl. "seven main rings (A, B, C, D, E, F, G)" + `[Memory Retrieval · rel x.xx]` lines) yet answered "It seems there may be a slight misalignment in our conversation—could you clarify what you're seeking? …like the rings of Saturn or cosmic themes…". The generator literally NAMED the referent in its own clarification but refused to resolve the anaphora "them"→rings. Not a delivery gap (that's fixed) — a **generation-stage referent-resolution gap**. Contributing cause: the 5 retrieved chunks were all *other* celestial poems (moons of Mars 0.658, Cartographer of Shadows 0.647, Titan's Flame 0.567, Epic Lyricism 0.551, Celestial Weapon 0.545), so "them" was genuinely multi-valued and the model's hedged-clarification move won over the immediate prior-turn context. EX3 (`7ed57c0e4c98`) "…about my first question about **saturn**…" bridged correctly (Saturn chunk surfaced at 0.589; output "Your journey from the rings of Saturn to poetic style…"). **Still open (refined)**: (1) retrieval-rank hygiene as above; (2) **anaphora resolution** — pronouns carry no lexical signal for the word-overlap retriever; candidate fixes: explicit pronoun→prior-subject binding in the chat context builder, or deprioritize retrieval chunks when Chat History already covers the subject. | ★★★ | 4–6 h | Delivery fixed; resolution open |
...
| 10 | **Reasoning effort curve (Illusion of Thinking method)** — companion probe: controllable *compositional complexity ladder* (puzzle generators that raise element count / depth while keeping logic identical, per Shojaee et al. 2025 — avoiding GSM-8K contamination) run across a reasoning-capable model + a plain model. Measure per-complexity accuracy *and* our trace shape: thinking tokens (`eval_count`/`token_count`), per-token entropy series, step durations, `2^H` branching factor. Questions: does the paper's counterintuitive *effort peak-then-decline* show up in our entropy/token telemetry? Can we flag the *overthinking regime* (correct answer early, then wasted exploration) as a trace-shape anomaly? Do we reproduce the three regimes (plain beats LRM at low complexity / LRM wins mid / both collapse)? | ★★★ | 4-6 h | ✓ **done 2026-08-23** — `backend/services/complexity_probe.py`: two generators — **arithmetic chain** (sequential +/- ops, complexity 2–10, 5 instances per level, left-to-right evaluation) and **Tower of Hanoi** (minimum moves = 2^N − 1, complexity 2–8, 3 instances per level, N ≥ 6 forces derivation). Per cell: exactness, entropy mean/p95/median 2^H, token count. Aggregation per-generator per-complexity: accuracy, mean tokens, mean entropy, mean branching, efficiency ratio (actual/optimal). Narrative auto-detects peak-then-decline signature. `POST/GET /api/probe/complexity` + `GET .../summary` + `GET .../export.csv`. `ComplexityLadderPanel.tsx`: model + generator selectors, three SVG line charts (accuracy/tokens/entropy vs complexity with optimal dashed line), per-model summary table, expandable cell detail. Wired into Tests tab below ReasoningProbePanel. researchRefs `reasoning-fragility` second entry (arXiv:2506.06941) already registered |
...
| 12 | **Weekly probe baseline cadence** — run the reasoning + complexity batteries on a schedule and accumulate the aggregates so fragility-drop / efficiency ratio become a comparable longitudinal series per model. **done 2026-09-05** — `tools/run_probe_baseline.py` (reads active model from `network.json` or CLI `--model/--provider`; fixed seed 42 so each week exercises the *same* problems; graceful per-battery failure, exit codes for the timer). systemd user units `mythic-probe-baseline.service`/`.timer` → **Mon 03:00 weekly** (`OnCalendar=Mon 03:00`, `Persistent=true`). Appends `{type, run_at, model, provider, seed, run_id, total, completed, failed, summary}` to `backend/data/probe_history.jsonl` (gitignored). Read-back: `GET /api/probes/history?limit=N&probe=reasoning|complexity` (newest-first, corrupt lines skipped, missing-file → `[]`). Smoke-tested live: reasoning battery appended a real GSM-Symbolic record (qwen3 100/100/100 0% fragility drop; DDC margin 0.093→0.023 across surface variants). **History-tab panel over `/api/probes/history` ✓ done 2026-09-05** — `ProbeHistoryPanel.tsx` on the History tab (above TraceTable): all/reasoning/complexity filter, per-record card with headline accuracy badge, expandable detail (reasoning: base/symbolic/noop accuracy + entropy/2^H/DDC-margin per variant + fragility drops; complexity: per-generator accuracy/tokens/entropy/2^H/eff table + narrative), refresh button, graceful empty state explaining the weekly cadence. | ★☆☆ | 2 h | Longitudinal data — the raw material for Phase 21 interpretation |

### Notes

- **Items 9-10 came from outside the observatory's own data** — Mirzadeh et al. 2024 (GSM-Symbolic) demonstrates the *behaviour* (reasoning collapse from irrelevant context); Shojaee et al. 2025 (Illusion of Thinking) demonstrates the *effort curve* (reasoning effort peaks then declines near the complexity-collapse point, with an overthinking regime at low complexity). Our pipeline is positioned to explain the *mechanism* of both: which stage — retrieval, classification, generation — carries the failure, and whether the paper's token-budget findings have trace-shape signatures we can flag (entropy series, `eval_count`, `2^H`, step durations). This is the observatory's core move: take a published behavioural finding and reproduce it instrumented. **Both items now implemented (2026-08-05 and 2026-08-23).** Next: cross-model runs on the complexity ladder to reproduce the three regimes (plain beats LRM at low complexity / LRM wins mid / both collapse). |
...
| 8 | **Mythic Layer** — name recurring phenomena as UI garnish (high entropy = "The Whispering Forest", self-reference = "The Mirror Pool"). Technical definition must stay visible beside the name. ✓ **done 2026-08-27** — `frontend/src/components/MythicLayer.tsx`: a self-nulling strip in the IntelligencePanel completed state that names a trace's measurable phenomena from already-captured telemetry — **The Whispering Forest** (high-entropy token ratio, with whisper/surge/torrent severity tiers calibrated to the live corpus: ratio ≥0.08 surge, ≥0.15 torrent, vs corpus median ~0.07 and cap ~0.28, so "torrent" is genuinely the top-decile outliers), **The Burning Bush** (median 2^H ≥ 4 — nearly every token had 4+ plausible continuations), **The Serpent's Crown** (single token entropy ≥2.0, reserved for the extreme peaks that cluster near 1.0 normally), **The Still Waters** (median 2^H ≤ 1.05 — deterministic, below the typical ~1.2 floor), **The Mirror Pool** (memory retrieval recovered this trace's own earlier artifact as grounding — reuses the correction-detector's self-ref signal). Every phenomenon carries its technical definition inline — the name is garnish, the number is truth. Verified `derivePhenomena` against 50 real traces + fabricated edge cases; `tsc --noEmit` clean. | ★☆☆ | 1 h | Cheap delight; guard truth over polish |
...
## Phase 21 — Interpretation / Diagnosis (the "so what?" workstream)

*Status: **DEFERRED (planned)** — *not started.** Added 2026-08-27 from a general critique of the Observatory the same night the Mythic Layer shipped. This phase attacks the Observatory's single biggest weakness, which every observability project eventually hits: the gap between *measuring* something and *knowing what the measurement means*.*

*The thesis: meaning is not downstream of measurement — it is downstream of **interpretation**. We already capture an enormous surface (entropy series, branching factor, DDC/LCC margins, mood/intent, retrieved/discarded chunks, stage durations, correction signals, StageDebate). What we do NOT do is converge those signals into a single *decision* for the operator. A number, on its own, tells you nothing about what to do. Interpretation is the stage we haven't built. This is distinct from Phase 14's diagnostic *probes* (which test the model) and Phase 18's phenomena *naming* (which adds texture): Phase 21 turns a deviation into a *recommendation*.*

*Guardrail (the failure mode to avoid): the wrong fix here is an LLM that narrates every panel in flowing prose. That is interpretation **theater** — it sounds meaningful and means nothing, and it betrays "truth over polish." The fix is not more language; it is **more decisions made checkable**. Every "story" must bottom out in: (a) a number the operator can verify, and (b) an action they can take or refuse. No story without both.*

| # | Item | Effort | Est. Time | Notes |
|---|------|--------|-----------|-------|
| 1 | **The "so what?" verdict card (the core)** — one collapsible card per trace/conversation that takes the *existing* signals (entropy, branching, retrieval similarity, DDC/LCC prompt margins, correction flags, StageDebate, root-cause heuristic, performance insights) and emits a single structured verdict: **What it is** (the measurement, exact value) / **Why it happened** (the stage that carried it — reuses causal tracing) / **What to check next** (a recommendation that ends in a verb, aimed at the *operator*, not the model). Not prose — a structured verdict with cited numbers. This is the bridge from "we measured a lot" to "we understood something, and here's what we'll watch next." | ★★★ | 3–4 h | Reuses findRootCause + PerformanceInsights + StageDebate + correctionDetector; the consolidation is the new work |
| 2 | **Every deviation resolves to a recommended action** — establish the rule that any surfaced anomaly (entropy spike, margin collapse, `used`-but-ignored retrieval, phase shift) must ship with its "what to do" as a first-class field, defaulting to a sensible action (clear context / re-run / trust / investigate). Detections without a recommendation are considered incomplete. | ★★☆ | 1.5 h | Convention + plumbing; prevents "the shiny number" trap |
| 3 | **Calibration grounding (metric → ground truth)** — extend the "near-zero entropy on canonical facts is a feature" validation into a standing discipline: every headline metric gets a small check where the ground truth is known and we verify the metric behaves. If high entropy does not reliably track wrong answers, then the phenomenon labels are *decoration* and we say so. | ★★☆ | 2 h | Truth over polish, applied to our own claims |
| 4 | **Correlation, not causation, everywhere** — keep gating verdicts behind minimum-N (already the pattern for entropy-vs-confidence at `r≈0` with `MIN_N` gate). Formalize: no causal claim leaves the Observatory without either a reproduced mechanism or an explicit "anecdotal / correlated" label. | ★☆☆ | 1 h | Mostly already done; make it a documented rule |
| 5 | **Published-reproduction standing discipline** — (GSM-Symbolic + Illusion-of-Thinking already done) make this a standing practice: pick a paper, reproduce its behaviour in our telemetry, and treat "we *cannot* see the mechanism" as a finding equal to seeing it. The instrument's claims earn credibility by surviving external checks. | ★★☆ | 2 h | Extends Phase 14 items 9–10 |
| 6 | **Input-side branching factor (operator ambiguity)** — measure the prompt/operator side of the loop: mood (interrogative/imperative), hedging, multi-intent density, prompt length — and correlate with the model's output entropy. First time the Observatory says something *true about the operator*, not just the model; the move that makes it more than a dashboard. *(overlaps Phase 18 #11 — consolidate)* | ★★☆ | 2 h | Closes the two-sided loop |

### Verification & Regression Checks

- Principles: every Phase 21 verdict must (a) cite a verifiable number, (b) name the carrying stage, (c) end in a recommended action (or a refusal-with-rule). No flowing-prose narration.
- Frontend: `npx tsc --noEmit` clean; the verdict card self-nulls like other panels when inputs are absent.
- Backend: pytest green; verdict derivation is deterministic (same trace → same verdict), no LLM in the loop unless explicitly gated.
