...
| 8 | **Context carry-over** — v1: the existing memory-retrieval stage treats prior exchanges in the same chat as retrievable context (no raw history injection). v2 (research): explicit truncated history injection per model family. ⚠ **live-observed failure 2026-08-06** (chat `53f3b337`): memory retrieval *works* but the model *ignores* it. EX2 ("write the lyrical narrative poem you suggested on the topic we were discussing") retrieved EX0's moons-of-Mars response at relevance **0.6179, `used: true`** — yet generated a generic shadow-cartography poem with no Mars. The ranked #1 chunk (0.6466) was the *previous exchange's own meta-commentary* ("Thank you for your clarification…"), polluting the topic selection; a follow-up correction (EX3) re-anchored to EX0 at 0.7896 and the model got it right. Verdict: this is a **used-but-ignored grounding failure** + **self-reference retrieval pollution**, not missing memory. v2 must include history injection AND a retrieval-rank hygiene fix (deprioritize chunks from the immediately-preceding exchange, or filter self-referential responses from the retrieval corpus). | ★★★ | 4–6 h | Research-heavy |
