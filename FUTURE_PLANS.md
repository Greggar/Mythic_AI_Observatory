...
| 11 | **Model-runner control layer (swap/load/unload per runner)** — for EVERY inference runner our system can route to (Docker Model Runner, Ollama, vLLM, TGI, llama.cpp-server / any OpenAI-compat), document + script the model lifecycle so multi-model test sessions work: how a model is loaded/unloaded/swapped, idle-timeout/eviction policy, VRAM scheduling behaviour when memory is full (hold-in-flight vs 503 vs spawn-failure), and the health/metrics endpoint the freeze watcher (`tools/backoffice_watch.py`) should sample. Seed (2026-08-06, DMR): Docker Model Runner v1.2.4 loads a model on first request, keeps it resident until another model is requested or a **5-minute idle timeout**, unloads then; the scheduler **holds in-flight requests until VRAM frees** rather than erroring (which looked like a "freeze" — a 90s+ hang with `CUDA busy` on spawn was the queue waiting for the resident 9B to idle-unload). CLI `docker model unload` evicts early; native API = `/models/create` (pull), `/models`, `/models/{ns}/{name}` GET/DELETE; OpenAI-compat at `/v1` + `/engines/v1`. Ollama/vLLM differ (Ollama keeps every requested model resident with no idle eviction; vLLM pools KV across a model set via continuous batching). Goal: a `per-model test session` script per runner — load → run the probe/suite → unload — so the Phase 14 test matrix doesn't stall on spawn contention. | ★★★ | 3-4 h | Unblocks reliable multi-model benchmarking on the backoffice (single-GPU, DMR-held queues) |
