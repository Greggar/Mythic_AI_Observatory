...
- **[Live finding] Used-but-ignored chat grounding (chat `53f3b337`, 2026-08-06).** EX2 "write the lyrical narrative poem you suggested on the topic we were discussing" retrieved EX0's moons-of-Mars response at relevance **0.6179 (`used: true`)** but generated a generic shadow-cartography poem with no Mars — the retrieved context was ignored. The ranked #1 chunk (0.6466) was the previous exchange's own meta-commentary ("Thank you for your clarification…"), a self-reference that polluted topic selection. The follow-up correction (EX3) re-anchored to EX0 at **0.7896** and the model bridged correctly to moons of Mars. DDC prompt margin told the arc: EX0 0.1108 → EX2 **0.0023** (elliptical prompt, all-minilm near-tie) → EX3 0.0659 (correction re-anchors the topic). **Observation lesson:** `trace.memory_retrieval` is always `null` — retrieval lives in `steps[4].metadata.retrieved_chunks`; reading the wrong field produced a false "0 chunks" diagnosis before the real read. (2026-08-06)
