...
- **[Backend] Complexity-ladder probe (Phase 14 #10, Illusion of Thinking method).** `services/complexity_probe.py`: two generators — **arithmetic chain** (sequential +/- operations evaluated left-to-right, complexity = operation count 2–10, 5 random instances per level, operands [1..15], post-filter keeps intermediates non-negative) and **Tower of Hanoi** (minimum moves = 2^N − 1, complexity = disc count 2–8, 3 instances per level, varying peg names; N ≥ 6 forces derivation with "do not just state a formula"). Per cell: exactness (reuses `_extract_answer` + `_mentions_exact` from reasoning_probe), entropy mean/p95/median 2^H, token count. Aggregation per-generator per-complexity: accuracy, mean tokens, mean entropy, mean branching factor, efficiency ratio (actual_tokens / optimal_tokens). Optimal token estimates: arithmetic = `3 × complexity + 6`, Hanoi = `max(12, 6 × complexity)`. Narrative auto-detects peak-then-decline signature (token count peaks then drops >15% on harder problems). In-memory store same pattern as reasoning_probe. `POST /api/probe/complexity` + `GET /api/probe/complexity/{run_id}` + `GET .../summary` + `GET .../export.csv`. (2026-08-23)
- **[Frontend] ComplexityLadderPanel (Phase 14 #10 UI).** Tests-tab panel below ReasoningProbePanel: model multi-select chips (3-level provider detection), generator toggles (Arithmetic Chain / Tower of Hanoi). Three SVG line charts — accuracy vs complexity, thinking tokens vs complexity (dashed optimal line overlay), entropy vs complexity — with solid lines for generator 1, dashed for generator 2, color-coded per model. Per-model summary table (gen, complexity, accuracy %, tokens, entropy, 2^H, efficiency ratio). Expandable cell detail. Narrative interpretation with bold highlights. ResearchPopover `reasoning-fragility` cites Shojaee et al. (arXiv:2506.06941). (2026-08-23)
